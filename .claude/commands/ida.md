...
```
> **xrefs vs cross-refs**: `xrefs` = 직접(1단계) 레퍼런스. `cross-refs` = `--depth`까지 재귀 체인 추적 + mermaid/DOT 그래프 출력.
...
- `--count N` / `--offset N` -- 리스트 명령어 페이징 (functions, strings, imports, exports)
- `--max N` -- 검색 명령어 결과 제한 (find_func, find_pattern, search-const, search-code, vtables)
